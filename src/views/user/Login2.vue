  ...
  <div class="login-container">
    <a-row class="login-main">
      ...
      <a-col :span="12" class="login-form-container">
        <div class="title">欢迎登录权星艺链后台</div>
        <a-form-model ref="form" :model="form" :rules="rules" class="form">
          ...
          <a-form-model-item prop="password">
            ...
            <a-button style="float: right;margin: 10px" @click="getMs" v-if="!msgLoading">获取验证码</a-button>
            <a-button style="float: right;margin: 10px" loading v-else>获取验证码</a-button>
          </a-form-model-item>
        </a-form-model>
...
<!--            <span v-else>登录中...</span>-->
    ...
    <a-row class="foot">
      ...
      <a-row type="flex" justify="center">
        ...
        <a href="javascript:void(0);" target="_blank" style="margin:0 5px;">权星艺链</a>
        版权所有
      </a-row>
    </a-row>
  </div>
...
export default {
  ...
  methods: {
    getMs () {
      ...
      getMs({ mobile: this.form.account }).then(res => {
        if (res.ok) {
    ...
    handleLogin () {
      this.loading = true
          this.$store.dispatch('loginPassword', { password: this.form.password, mobile: this.form.account }).then(() => {
            this.$router.push({ path: this.redirect || '/' })
          }).finally(() => {
            this.loading = false
          })
      // console.log('router to:', this.redirect)
      // this.$router.push({ path: this.redirect || '/' })
